...
const CrmRecord = ({ data, isSelected, disabled, onCheckboxChange }) => {
  return (
    <div className="relative flex w-1/4 flex-shrink flex-grow flex-col overflow-hidden rounded-lg border border-gray-300 p-3  py-3 pb-5 pt-9 lg:w-1/4  xl:w-1/4">
      ...
      <figure className="relative mb-1 flex flex-1 flex-col items-center">
        ...
        <figcaption className="flex flex-col items-center pt-8 leading-6 text-gray-900">
          <div className="text-sm line-clamp-2 max-h-10 overflow-hidden text-center font-medium text-[#6521AD]">
            <Link
              href={`/journalist-profile/${data.intJournalistId}`}
              target="_blank"
            >
          ...
          <span className="pt-2 text-center text-sm font-medium leading-none text-gray-600">
            {data?.vchOutletName}
          </span>
          <div className="mt-1">
