...
import ByShowRecord from "./ByShowRecord";
...
const JournalistByShow = ({ outletId, id, name }) => {
  ...
  const [active, setActive] = useState("Shows");
  const [postDataToCrm, setPostDataToCrm] = useState([]);
  const url = `${BROADCASTBYSHOW}`;
  ...
  const { isdata, loading, loadMore, loadingMore, noMore } =
    useLeftJournoSearch(
      ...
      url + `?OutletId=${id}&pageSize=60&`,
      searchDeps
    );
  //url + `?OutletId=${id}&GenreFilter=${abc}&pageSize=60&`,
  console.log(selectedFilters, "check id here...");
  ...
  //   const allIds = journalistResult?.map((journalist) => journalist.intShowId);
      ...
      <div className="section w-11/12 p-6 pb-6 pl-8 pr-0 pt-0">
            ...
            <div className="flex w-full flex-wrap  self-start [&>*:last-child]:flex-none [&>*:nth-child(n-2)]:flex-none">
              {journalistResult && journalistResult?.length > 0
                ...
                : loading
                  ? "Loading..."
                  : "No Record Found"}
              {/* <div className="left mt-1 text-center text-xl leading-none  text-gray-500">
                Coming Soon{" "}
              </div> */}
        ...
        {isOpen && (
          <BuildCampaign
            ...
            Editor_Columnis_Bureau_Filter="show"
            open={isOpen}
...
export default JournalistByShow;
