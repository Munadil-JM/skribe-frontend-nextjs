...
const Columnist = ({ outletId, id, name }) => {
  ...
  const [active, setActive] = useState("Columnist");
  ...
  const { isdata, loading, loadMore, loadingMore } = useLeftJournoSearch(
    ...
    url +
      `?OutletFilter=${id}&Editor_Columnis_Bureau_Filter=Columnis&SearchFilter=${searchByText}&pageSize=60&`,
    searchDeps
  );
  ...
  const addInCRM = () => {
    if (selectedJournalists.length === 0) {
      warning("Please select Journalist to add in crm", "warning");
      //dispatch(WarningAlert("please select Journalist to add in crm"));
    } else if (selectedJournalists?.length > 5) {
      warning("you can not add to crm more than 5", "warning");
    } else {
      ...
      userService
        ...
        .then((dataSubmit) => {
          ...
          if (dataSubmit?.response?.status === "Ok") {
            success(
              dataSubmit?.insertedCount +
                " Added to CRM out of " +
                dataSubmit?.totalCount +
                " beacause " +
                (dataSubmit?.totalCount - dataSubmit?.insertedCount) +
                " is already added to CRM",
              "success"
            );
