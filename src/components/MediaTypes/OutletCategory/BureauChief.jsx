...
const BureauChief = ({ outletId, id, name }) => {
  ...
  const [active, setActive] = useState("Bureau Chief");
  ...
  const { isdata, loading, loadMore, loadingMore } = useLeftJournoSearch(
    ...
    url +
      `?OutletFilter=${id}&Editor_Columnis_Bureau_Filter=Bureau&SearchFilter=${searchByText}&pageSize=60&`,
    searchDeps
  );
  ...
  const handleSelectAll = () => {
    if (selectedJournalists.length === journalistResult.length) {
