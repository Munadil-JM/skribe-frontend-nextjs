import { trendingSpokesperson, spokespersonAPI_Article } from "../../constants";
import { queryKeys } from "../../reactQuery/constants";
import axios from "axios";
import { useQuery } from "react-query";

async function trendingSpokespersonApi(keyword) {
  let token = localStorage.getItem("userToken");
  if (keyword !== undefined) {
    let result = await axios.post(
      `${trendingSpokesperson}${encodeURIComponent(keyword)}`,
      {
        headers: {
...
export function useTrendingSpokespersonApi(keyword) {
  const fallback = [];
  const { data = fallback, isLoading } = useQuery(
    [queryKeys.TRENDINGSPOKESPERSON, keyword],
    () => trendingSpokespersonApi(keyword)
  );

  let spokespersonApiData = data;
  let isLoadingSpokespersonApi = isLoading;
  return { spokespersonApiData, isLoadingSpokespersonApi };
}

/* async function for aggregation influencer */
export async function spokespersonApiAggregateFn(keyword, arr, buckets) {
  if (!arr.length)
  ...
  for (let ele of arr) {
    try {
      ...
      if (aggregations[`spokesperson_count_${n}`] === undefined) {
        const v = buckets.find(({ key, doc_count }) =>
          ele === key ? doc_count : undefined
        );
        aggregations[`spokesperson_count_${n}`] = { doc_count: v["doc_count"] };
      }
    } catch (error) {}
  }
...
export default async function spokesperson_article({
  ...
  if (!pageParam)
    processed_url = `${spokespersonAPI_Article}${encodeURIComponent(keyword)}`;
  else
    processed_url = `${spokespersonAPI_Article}${encodeURIComponent(
      keyword
